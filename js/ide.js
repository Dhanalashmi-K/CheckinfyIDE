...
function handleResult(data) {
  ...
  if(stdout=="")
   ...
   $runBtn.removeClass("loading");
    ...
    stdoutEditor.setValue(stderr);
    stdoutEditor.setValue(compile_output);
    ...
    if (snippet_id.length == 36) {
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                stdoutEditor.setValue(decode(data["stderr"]));
                stdoutEditor.setValue(decode(data["compile_output"]));
...
$(document).ready(function () {
  ...
  .change();
    ...
    require(["vs/editor/editor.main", "monaco-vim", "monaco-emacs"], function (ignorable, MVim, MEmacs) {
        ...
        layout1 = new GoldenLayout(layoutConfig1, $("#site-content1"));
        ...
        layout1.registerComponent("stdin", function (container, state) {
        ...
        layout1.registerComponent("stdout", function (container, state) {
      ...
      layout1.on("initialised", function () {
            $(".monaco-editor")[0].appendChild($("#editor-status-line")[0]);
            if (getIdFromURI()) {
                loadSavedSource();
            } else {
